...
using CongestionTaxCalculator.Infrastructure.Common.Utilities;
using CongestionTaxCalculator.Domain.Common;
using CongestionTaxCalculator.Domain.Common.Utilities;

namespace CongestionTaxCalculator.Infrastructure.DbContexts.Sql.SqlServer
{
    public class ApplicationDbContext : DbContext
    {
        private readonly IMediator _domainBus;

        public ApplicationDbContext(DbContextOptions<ApplicationDbContext> options, IMediator domainBus)
        : base(options)
        {
        ...
        private async Task PublishEventsAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var aggregateRoot in aggregateRoots)
            {
                ...
                {
                    await _domainBus.Publish(domainEvent, cancellationToken);
                }
